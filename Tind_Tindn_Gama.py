...
Gama = pd.concat((pd.read_csv(file, encoding ='latin1', sep="\s+", index_col= None, header = None, usecols = [7]).assign(filename = file) for file in sinais), ignore_index=False, axis = 1)
...
X = pd.concat([Tind, Gama], axis = 1, ignore_index=True)
y = Tproc
